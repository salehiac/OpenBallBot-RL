...
def main(args, seed=None):
    ...
    with torch.no_grad():
        ...
        env = make_ballbot_env(
            gui=True,
            terrain_type=model.terrain_type
            if not args.override_terrain_type else args.override_terrain_type,
            log_options={
                "cams": False,
                "reward_terms": True
            },
        ...
        if args.override_terrain_type:
            print(
                colored(
                    f"policy was trained on terrain type {model.terrain_type}, but it will be tested on terrain type {args.override_terrain_type}",
                    "yellow",
                    attrs=["bold"]))
        else:
            print(
                colored(
                    f"policy was trained on terrain type {model.terrain_type} and will be tested on the same terrain type.",
                    "yellow",
                    attrs=["bold"]))
